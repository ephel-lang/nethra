...
object Binding {
    ...
    private val sig: Parser<Char, Cst.Binding> =
        SIG thenRight ID thenLeft COLON then Term() map { Cst.Binding.Signature(it.first, it.second) }

    private val def: Parser<Char, Cst.Binding> =
        DEF thenRight ID thenLeft EQUAL then Term() map { Cst.Binding.Definition(it.first, it.second) }

    operator fun invoke(): Parser<Char, Cst.Binding> = sig or def
