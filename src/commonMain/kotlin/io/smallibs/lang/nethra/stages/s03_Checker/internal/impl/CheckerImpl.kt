...
import io.smallibs.lang.nethra.stages.s03_Checker.internal.Proof.Context
...
class CheckerImpl<C>(
    ...
    override fun Bindings<C>.check(
        term: Term<C>,
        type: Term<C>,
    ): Boolean = reduce(type).let { reducedType ->
        println("[↑]${this.prettyPrint()} ⊢ ${term.prettyPrint()} : ${reducedType.prettyPrint()} / ?").let {
            // Unit.let {
            val i = Context(this, reducedType)
            val r = term.run(i) || term.fallback(i) || throw CompilationException.CannotCheck(term, reducedType)
            println("[↑]${this.prettyPrint()} ⊢ ${term.prettyPrint()} : ${reducedType.prettyPrint()} / $r")
            r
        }
    }
    ...
    // ----------------
    // Γ, x : T ⊢ x : T
    override fun Id<C>.run(i: Context<C>) =
    ...
    override fun Lit<C>.run(i: Context<C>) = with(inferenceGenerator(this@CheckerImpl)) {
    ...
    override fun Lambda<C>.run(i: Context<C>) = when (i.type) {
        is Pi -> {
    ...
    override fun Apply<C>.run(i: Context<C>) = with(inferenceGenerator(this@CheckerImpl)) {
    ...
    // --------------------------
    // Γ ⊢ A,B : Σ(x:M).N
    override fun Couple<C>.run(i: Context<C>) = when (i.type) {
    ...
    // ----------------
    // Γ ⊢ fst p : M
    override fun Fst<C>.run(i: Context<C>) = with(inferenceGenerator(this@CheckerImpl)) {
    ...
    // ----------------------
    // Γ ⊢ snd p : N[x=fst p]
    override fun Snd<C>.run(i: Context<C>) = with(inferenceGenerator(this@CheckerImpl)) {
        when (val type = i.gamma.infer(term)) {
    ...
    // -----------------
    // Γ ⊢ inl A : M + N
    override fun Inl<C>.run(i: Context<C>) = when (i.type) {
    ...
    // -----------------
    // Γ ⊢ inr A : M + N
    override fun Inr<C>.run(i: Context<C>) = when (i.type) {
    ...
    // -----------------------------------------------
    // Γ ⊢ case a l r : C
    override fun Case<C>.run(i: Context<C>) = with(inferenceGenerator(this@CheckerImpl)) {
        when (val type = i.gamma.infer(term)) {
    ...
    // ---------------------
    // Γ ⊢ fold A : rec(x).N
    override fun Fold<C>.run(i: Context<C>) = when (i.type) {
    ...
    override fun Unfold<C>.run(i: Context<C>) = with(inferenceGenerator(this@CheckerImpl)) {
    ...
    // ----------------
    // Γ, x : T ⊢ x : T
    override fun Hole<C>.run(i: Context<C>) =
        i.gamma.getSignature(this@run.value)?.let { type -> i.gamma.congruent(type, i.type) } ?: false
    ...
    private fun Term<C>.fallback(i: Context<C>) = when (i.type) {
