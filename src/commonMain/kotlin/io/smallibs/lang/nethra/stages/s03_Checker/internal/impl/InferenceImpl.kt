...
class InferenceImpl<C>(
    ...
    // ---------------------
    // Γ ⊢ Type_i : Type_i+1
    override fun Ast.Term.Type<C>.run(i: Bindings<C>): Ast.Term<C> = type(level + 1)

    //
    // ----------------
    // Γ, x : T ⊢ x : T
    override fun Ast.Term.Id<C>.run(i: Bindings<C>): Ast.Term<C> =
        i.getSignature(value) ?: throw CompilationException.Unbound(this)

    // l ∈ int         l ∈ char          l ∈ string
    // -----------     -------------     -------------
    // Γ ⊢ l : int     Γ ⊢ l : char      Γ ⊢ l : string
    override fun Ast.Term.Lit<C>.run(i: Bindings<C>): Ast.Term<C> = when (this.literal) {
    ...
    override fun Ast.Term.Pi<C>.run(i: Bindings<C>): Ast.Term<C> {
        i.infer(bound)
        return i.setSignature(n, bound).infer(body)
    }

    // Γ, x : A ⊢ B : T          Γ, x : A ⊢ B : T
    // ---------------------     ---------------------
    // Γ ⊢ λ(x).B : Π(x:A).T     Γ ⊢ λ{x}.B : Π{x:A}.T
    override fun Ast.Term.Lambda<C>.run(i: Bindings<C>): Ast.Term<C> = hole(newVariable()).let { hole ->
        pi(n, hole, i.setSignature(hole.value, hole).infer(body.substitute(n to hole)), implicit)
    }

    // Γ ⊢ f : Π(x:M).N   Γ ⊢ e : M      Γ ⊢ f : Π{x:M}.N   Γ ⊢ e : M      Γ ⊢ f : Π{x:M}.N   Γ, v:M ⊢ f {v} e : N
    // ----------------------------      ----------------------------      -----------------------------------------
    // Γ ⊢ f e : N[x=e]                  Γ ⊢ f {e} : N[x=e]                Γ ⊢ f e : N
    override fun Ast.Term.Apply<C>.run(i: Bindings<C>): Ast.Term<C> = when (val type = i.infer(abstraction)) {
        is Ast.Term.Pi -> if (implicit == type.implicit) {
            if (i.check(argument, type.bound)) {
                type.body.substitute(type.n to argument)
            } else {
                throw CompilationException.CannotCheck(this, type)
            }
    ...
    // ----------------
    // Γ ⊢ fst p : M
    override fun Ast.Term.Fst<C>.run(i: Bindings<C>): Ast.Term<C> = when (val type = i.infer(term)) {
        is Ast.Term.Sigma -> type.bound
        else -> throw CompilationException.CannotCheck(term, type)
    }

    // Γ ⊢ p : Σ(x:M).N
    // ----------------------
    // Γ ⊢ snd p : N[x=fst p]
    override fun Ast.Term.Snd<C>.run(i: Bindings<C>): Ast.Term<C> = when (val type = i.infer(term)) {
        is Ast.Term.Sigma -> type.body.substitute(type.n to fst(term))
        else -> throw CompilationException.CannotCheck(term, type)
    }
    ...
    // ---------------------
    // Γ ⊢ A | B : T
    override fun Ast.Term.Disjunction<C>.run(i: Bindings<C>): Ast.Term<C> = or(i.infer(lhd), i.infer(rhd))

    // Γ ⊢ A : M
    // -----------------
    // Γ ⊢ inl A : M + N
    override fun Ast.Term.Inl<C>.run(i: Bindings<C>): Ast.Term<C> = or(i.infer(term), hole(newVariable()))

    // Γ ⊢ A : N
    // ------------------
    // Γ ⊢ inr A : M + N
    override fun Ast.Term.Inr<C>.run(i: Bindings<C>): Ast.Term<C> = or(hole(newVariable()), i.infer(term))

    // Γ ⊢ a : A + B   Γ ⊢ l : A -> C   Γ ⊢ r : B -> C
    // -----------------------------------------------
    // Γ ⊢ case a l r : C
    override fun Ast.Term.Case<C>.run(i: Bindings<C>): Ast.Term<C> = when (val type = i.infer(term)) {
    ...
    // ----------------
    // Γ ⊢ rec(x).A : T
    override fun Ast.Term.Rec<C>.run(i: Bindings<C>): Ast.Term<C> = hole(newVariable()).let { hole ->
