...
class BindingNominalSpec : StringSpec({
    ...
    "[parser] sig test : Int -> Int " {
        (Binding() thenLeft eos())(string("sig test : Int -> Int")).get()?.prettyBinding() shouldBe "sig test : Int -> Int"
    }

    "[parser] def test = Int -> Int " {
        (Binding() thenLeft eos())(string("def test = Int -> Int")).get()?.prettyBinding() shouldBe "def test = Int -> Int"
    }
